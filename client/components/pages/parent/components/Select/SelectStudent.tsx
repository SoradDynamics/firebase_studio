...
const SelectStudentComponent: React.FC<SelectStudentComponentProps> = ({
  ...
  return (
    <div className="w-full mb-6"> {/* Added mb-6 for spacing if used standalone */}
      <label htmlFor="student-select-component" className="block text-sm font-medium text-gray-700 mb-1.5">
        {label}
      </label>
      <div className="relative">
        <select
          ...
          className={`
            w-full pl-3 pr-10 py-2.5 text-base
            border border-gray-300 bg-white text-gray-800
            rounded-md shadow-sm 
            focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500
            appearance-none
            ${!hasStudents || isLoading ? 'bg-gray-100 cursor-not-allowed' : 'hover:border-gray-400'}
      ...
      {!hasStudents && !isLoading && (
        <p className="mt-2 text-xs text-gray-500">No students linked to your account.</p>
      )}
    </div>
  );
};
